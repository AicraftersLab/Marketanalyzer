...
def load_prophet():
    try:
        ...
        return Prophet, plot_plotly
    except Exception as e:
        st.error(f"Error loading Prophet: {str(e)}")
        return None, None
...
if st.session_state.data_loaded and not st.session_state.dataFrameyahoo.empty:
    ...
    with tab4:
        ...
        # Load Prophet only when the Predictions tab is accessed
        Prophet, plot_plotly = load_prophet()
        
        if Prophet is None:
            st.error("❌ Prophet could not be loaded. Please check deployment logs.")
            st.info("This is likely due to Prophet installation issues on Streamlit Cloud.")
        else:
            try:
